...
class MinimalSubscriber(Node):
    ...
    def move_arm(self):
        ...
        dist_x = tool0_x - boxes_coordinates[2][0]
        dist_y = tool0_y - boxes_coordinates[2][1]
        dist_z = tool0_z - boxes_coordinates[2][2]
        ...
        while (not self.in_correct_joints):
            ...
            self.get_logger().info(f"Moving to {{joint_positions: {list(initial_pose)}}}")
            moveit12.move_to_configuration(initial_pose)
            self.in_correct_joints = True
            time.sleep(4)
        ...
        if self.in_front : 
            ...
            while (not self.in_correct_joints):
                ...
                self.get_logger().info(f"Moving to {{joint_positions: {list(initial_pose)}}}")
                moveit13.move_to_configuration(drop_pose)
                self.in_correct_joints = True
                time.sleep(4)
            ...
            while (not self.in_correct_joints):
                ...
                self.get_logger().info(f"Moving to {{joint_positions: {list(initial_pose)}}}")
                moveit14.move_to_configuration(initial_pose)
                self.in_correct_joints = True
                time.sleep(4)
        ...
        while (not self.in_correct_joints):
            ...
            self.get_logger().info(f"Moving to {{joint_positions: {list(drop_pose)}}}")
            moveit15.move_to_configuration(drop_pose)
            self.in_correct_joints = True
            time.sleep(4)
        ...
        while (not self.in_correct_joints):
            ...
            self.get_logger().info(f"Moving to {{joint_positions: {list(initial_pose)}}}")
            moveit16.move_to_configuration(initial_pose)
            self.in_correct_joints = True
            moveit16.wait_until_executed()
